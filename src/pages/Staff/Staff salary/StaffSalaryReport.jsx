...
const StaffSalaryReport = () => {
  ...
  useEffect(() => {
    // Fetch the salary data when month or year changes
    fetchSalaryData();
  ...
  const fetchSalaryData = async () => {
    try {
      const { data } = await fetchSalariesByMonthAndYear(month, year);
      setSalaryData(data);
    } catch (error) {
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try {
      await updateSalaryStatus(id, { paid: newStatus });
      setSalaryData(salaryData.map(item =>
        item._id === id ? { ...item, paid: newStatus } : item
      ));
    } catch (error) {
        ...
        <div className="flex flex-col">
          ...
          >
            ...
            {Array.from({ length: 10 }).map((_, index) => (
              <option key={index} value={currentYear - 5 + index}>{currentYear - 5 + index}</option>
            ))}
          </select>
        </div>
          ...
          <tbody>
            {filteredData.map((item) => (
              <tr key={item._id} className="border-t">
                <td className="px-4 py-2 text-center">{item.staff._id}</td>
                <td className="px-4 py-2 text-center">
                  <img
                    src={`${BASE_URL}/${item.staff.image}`}
                    alt={`user_pic`}
                    className="w-12 h-12 rounded-full mx-auto"
                  />
                </td>
                <td className="px-4 py-2">{item.staff.name}</td>
                <td className="px-4 py-2 text-right">{item.salary.toFixed(2)}</td>
                <td className="px-4 py-2 text-center">
                  <button
                    className={`px-4 py-2 rounded-md ${
                    ...
                    onClick={() => toggleStatus(item._id, item.paid)}
                  >
