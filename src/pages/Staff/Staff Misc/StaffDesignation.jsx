...
import { FaPlus, FaTrashAlt, FaEdit } from 'react-icons/fa';
import Button from '../../../components/Button';
import DesignationModal from './StaffModal2';
import {
  fetchDesignations,
  addDesignation,
  updateDesignation,
...
const StaffDesignation = () => {
  ...
  const loadDesignations = async () => {
    try {
      ...
      console.error('Failed to fetch designations', error.message);
    }
  };
  ...
  const handleSaveDesignation = async (designation) => {
    try {
      if (isEditMode) {
        await updateDesignation(designation._id, designation);
      ...
      console.error('Failed to save designation', error.message);
  ...
  const handleDeleteDesignation = async (id) => {
    try {
      ...
      console.error('Failed to delete designation',  error.message);
  ...
  return (
    <div className="mx-auto p-4 mt-10 mb-4 overflow-y-auto">
      ...
      <table className="min-w-full bg-white">
        ...
        <tbody>
          {designations.length > 0 ? (designations.slice(0, rowsPerPage).map((designation) => (
            <tr key={designation._id}>
              <td className="py-2 px-4 border border-gray-200">{designation._id}</td>
              <td className="py-2 px-4 border border-gray-200">{designation.title}</td>
              <td className="py-2 px-4 border border-gray-200">{new Date(designation.createdAt.slice(0, 10)).toLocaleDateString()}</td>
              <td className="py-2 px-4 border border-gray-200">
                <div className="flex space-x-2">
                  <button
                    type="button"
