...
const ExpenseList = () => {
  ...
  const  handleDelete = async (id) => { 
    ...
    if (confirmed) {
      try {
        ...
        toast.error('Failed to delete expense data');
      }
    }
   }
  ...
  useEffect(() => {
    fetchData();
  }, [searchCategory, startDate, endDate, rowsPerPage, currentPage]);
    ...
    <div className="mx-auto p-4 mt-10 mb-4 overflow-y-auto">
      ...
      <div className="flex justify-between items-center bg-[#5D5B10] p-4">
        ...
        <Button
          ...
          bgColor="bg-[#5D5B10] hover:bg-[#5D5B00]"
          textColor="text-white border border-green-500"
          onClick={() => {/* Handle Add New Expense */}}
        />
      </div>
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody>
            {expenses.slice(0, rowsPerPage).map((expense, index) => (
              <tr key={expense.id}>
                <td className="py-2 px-4 border border-gray-200">{index + 1}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.date.slice(0,10)}</td>
                <td className="py-2 px-4 border border-gray-200">{expense._id}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.category.name}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.account.title}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.chequeNo}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.description}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.paymentMethod}</td>
                <td className="py-2 px-4 border border-gray-200">{expense.amount}</td>
                <td className="py-2 px-4 border border-gray-200">
                {expense.file ? (
                  <a href={expense.file} target="_blank" rel="noopener noreferrer" download>
                    <FaPrint className="text-blue-500 cursor-pointer" size={20} />
                  </a>
                ) : (
                  <MdPrintDisabled className="text-red-500" size={24} />
                )}
                </td>
                <td className="py-2 px-4 border border-gray-200">
                <button
